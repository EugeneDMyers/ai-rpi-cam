 ...
 *
 * kipr_post_rpocessing_stage.cpp - IMX500 post processing stage base class
 */
#include <chrono>
#include <cmath>
#include <fcntl.h>
...
#include "kipr_post_processing_stage.hpp"
...
KIPRPostProcessingStage::KIPRPostProcessingStage(RPiCamApp *app)
...
KIPRPostProcessingStage::~KIPRPostProcessingStage()
{
	if (device_fd_ >= 0)
		close(device_fd_);
}

void KIPRPostProcessingStage::Read(boost::property_tree::ptree const &params)
{
...
Rectangle KIPRPostProcessingStage::ConvertInferenceCoordinates(const std::vector<float> &coords,
...
void KIPRPostProcessingStage::SetInferenceRoiAuto(const unsigned int width, const unsigned int height) const
{
...
void KIPRPostProcessingStage::ShowFwProgressBar()
{
	if (fw_progress_.is_open() && fw_progress_chunk_.is_open())
	{
		std::thread progress_thread { &KIPRPostProcessingStage::doProgressBar, this };
...
void KIPRPostProcessingStage::doProgressBar()
{
	while (1)
	{
